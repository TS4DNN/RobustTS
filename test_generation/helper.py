...
def perturb_image_mnist(xs, img):
    if xs.ndim < 2:
        xs = np.array([xs])
    # batch = len(xs)
    # tile = [len(xs)] + [1] * (xs.ndim + 1)
    # print(tile)
    # imgs = np.repeat(img, 2).reshape(-1, 28, 28, 1)
    # imgs = img.copy()
    imgs = np.array([img for i in range(len(xs))])
    ...
    # print(xs)
    # x_pos, y_pos, grey = xs
    # imgs[0, x_pos, y_pos, 0] = (grey - 0.1307) / 0.3081
    # imgs[0, x_pos, y_pos, 0] = grey
    for i in range(len(imgs)):
        perturbation = xs[i]
        single_img = imgs[i]
        # print(perturbation)
        x_pos, y_pos, grey = perturbation
        # print(single_img.shape)
        single_img[x_pos, y_pos] = grey
        imgs[i] = single_img
    # for x in xs:
    #     # print(x)
    #     # pixels = np.split(x, len(x) / 5)
    #     # pixels = np.split(x, len(x) // 5)
    #     pixels = np.split(x, len(x) // 4)
    #
    #     for pixel in pixels:
    #         # print(pixel)
