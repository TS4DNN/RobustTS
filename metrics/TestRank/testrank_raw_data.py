...
def test_rank_prior(x, y, x_candidate, y_candidate, model, save_path, args):
    ...
    # latents = x_test.reshape(-1, 784)[:, :10]
    ...
    for p_budget in p_budget_lst:
        ...
        # output and logging
        # out_file = os.path.join(save_path, 'gnn_result.csv')
...
def test_rank_selection(x, y, x_candidate, y_candidate, model, budget, args="", data_type="mnist"):
    x_test = np.concatenate((x, x_candidate))
    y_test = np.concatenate((y, y_candidate))
    if data_type == "mnist":
        latents = x_test.reshape(-1, 784)
...
if __name__ == "__main__":
    ...
    # selected_index = test_rank_selection(x_test[:9000], y_test[:9000], x_test[:1000], y_test[:1000], model, 100)
    # prediction = model.predict(x_test).argmax(axis=1)
    # selected_prediction = prediction[selected_index]
    # wrong_num = len(np.where(selected_prediction != y_test[selected_index])[0])
    # print("fault number: ", wrong_num)
    ...
    selected_index = test_rank_selection(x_final[:1000], y_final[:1000], x_final[1000:], y_final[1000:], model, 100, data_type="cifar10")
    prediction = model.predict(x_final).argmax(axis=1)
    selected_prediction = prediction[1000:][selected_index]
    print(selected_prediction)
    print(y_final[1000:][selected_index])
    wrong_num = len(np.where(selected_prediction != y_final[selected_index])[0])
    print("fault number: ", wrong_num)
