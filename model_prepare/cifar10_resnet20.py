...
from tensorflow.keras.layers import Conv2D, Dense, Input, add, Activation, MaxPooling2D, AveragePooling2D, GlobalAveragePooling2D, Dropout
from tensorflow.keras.layers import BatchNormalization
from tensorflow.keras import optimizers, regularizers
from tensorflow.keras.models import Sequential, Model, load_model

from tensorflow.keras.layers import Convolution2D, MaxPooling2D, Input, Dense, Activation, Flatten,MaxPool2D, Dropout
...
def resnet20_drop(input_shape, num_classes, drop_rate=0.2, drop=False):
    input_tensor = Input(shape=input_shape)
    weight_decay = 1e-6
    stack_n = 3
    def residual_block(intput, out_channel, drop=drop, increase=False):
        if increase:
            stride = (2, 2)
        else:
            stride = (1, 1)
...
if __name__ == '__main__':
    ...
    model.summary()
    model.save("../models/cifar10/resnet20_drop.h5")
